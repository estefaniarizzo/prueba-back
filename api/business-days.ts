...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!daysParam && !hoursParam) {
    return res.status(400).json({ error: 'InvalidParameters', message: 'At least one of days or hours must be provided' });
  }
  ...
  if ((daysParam && (isNaN(days) || days < 0)) || (hoursParam && (isNaN(hours) || hours < 0))) {
    return res.status(400).json({ error: 'InvalidParameters', message: 'days and hours must be positive integers' });
  }
  ...
  try {
    ...
    if (dateParam) {
      if (!dateParam.endsWith('Z')) {
        return res.status(400).json({ error: 'InvalidParameters', message: 'date must be an ISO 8601 string in UTC ending with Z' });
      }
      const parsed = DateTime.fromISO(dateParam, { zone: 'utc' });
      if (!parsed.isValid) {
        return res.status(400).json({ error: 'InvalidParameters', message: 'date must be a valid ISO 8601 UTC string with Z' });
      }
    ...
    return res.status(503).json({ error: 'InternalError', message: err?.message ?? 'Unknown error' });
  }
}
